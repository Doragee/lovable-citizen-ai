...
민원 내용을 분석하여 핵심 요점을 1줄로 간단명료하게 요약하고, 요약된 내용을 기반으로 가장 적합한 담당 부서를 찾아주세요.

요약 작성 지침:
- 민원의 핵심 문제나 요청사항을 간결하게 표현 (예: "출장 전일 숙박비 지급 가능 여부", "도로 파손 보수 요청")
- 8단어 이내로 작성 (더 간결하게)
- 구체적이고 실행 가능한 내용으로 작성

부서 선택 지침:
- 작성한 요약 내용을 기반으로 부서를 선택
- 반드시 아래 제공된 부서 목록에서만 정확히 선택
- 요약된 민원 내용과 부서의 주요업무/키워드를 매칭하여 가장 적합한 부서 선택
...
{"summary": "핵심요점 1줄 요약", "department": "부서명"}
